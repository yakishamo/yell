...
Command *parse_command(char *str) {
  ...
  com->tokens[0].value = malloc(sizeof(char) * (len + 1));
  strcpy(com->tokens[0].value, tok_str);
  com->size++;
  ...
  while(1) {
    for(int i = com->size; i < com->capacity; i++) {
      ...
      len = strlen(tok_str);
      com->tokens[i].value = malloc(sizeof(char) * (len + 1));
      strcpy(com->tokens[i].value, tok_str);
      com->size++;
    }
    com->capacity += TOK_SIZE;
